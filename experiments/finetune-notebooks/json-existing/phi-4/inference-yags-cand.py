...
test_prompts_df = pd.read_json('../fn1.7-test-yags-prompts.jsonl', lines=True)
...
for i, example in enumerate(test_dataset['messages'][len(predictions):]):
    ...
    print(outputs)
    output_text = tokenizer.decode(outputs[0]).split('<|im_start|>assistant<|im_sep|>')[-1]
    predictions.append(output_text)

    # Write output to file
    if i % 10 == 0:
        pd.DataFrame(predictions, columns=["predictions"]).to_json("fn1.7-yags-test-predictions-temp.jsonl", lines=True, orient="records")


# Save the predictions
test_prompts_df["predictions"] = predictions
test_prompts_df.to_json("fn1.7-yags-test-cand-predictions.jsonl", lines=True, orient="records")
