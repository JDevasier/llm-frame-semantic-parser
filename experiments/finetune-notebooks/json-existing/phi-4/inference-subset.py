...
parser.add_argument("--frac", type=str, default="01")
args = parser.parse_args()

max_seq_length = 2048
model, tokenizer = FastLanguageModel.from_pretrained(
    # model_name="llama-3.1-70b-fsp-ft",
    model_name=f"phi-4-fsp-ft-{args.frac}",
...
for i, example in enumerate(test_dataset['messages'][len(predictions):]):
    ...
    if i % 10 == 0:
        pd.DataFrame(predictions, columns=["predictions"]).to_json(f"fn1.7-test-{args.frac}-predictions-temp.jsonl", lines=True, orient="records")


# Save the predictions
test_prompts_df["predictions"] = predictions
test_prompts_df.to_json(f"fn1.7-test-{args.frac}-predictions.jsonl", lines=True, orient="records")
