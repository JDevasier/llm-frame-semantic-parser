...
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    model_name="qwen-2.5-72b-fsp-cand-ft",
...
# Check if file exists, if so, load the predictions
if os.path.exists("fn1.7-test-predictions-temp.jsonl"):
    predictions = pd.read_json("fn1.7-test-predictions-cand-temp.jsonl", lines=True)["predictions"].tolist()
...
for i, example in enumerate(test_dataset['messages'][len(predictions):]):
    ...
    if i % 10 == 0:
        pd.DataFrame(predictions, columns=["predictions"]).to_json("fn1.7-test-predictions-cand-temp.jsonl", lines=True, orient="records")

pd.DataFrame(predictions, columns=["predictions"]).to_json("fn1.7-test-predictions-cand-final.jsonl", lines=True, orient="records")

# Save the predictions
test_prompts_df["predictions"] = predictions
test_prompts_df.to_json("fn1.7-test-predictions-cand.jsonl", lines=True, orient="records")
